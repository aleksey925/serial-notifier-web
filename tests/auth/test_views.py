...
@pytest.mark.asyncio
async def test_login__send_req_with_valid_data__success(create_access_token_mock, async_client, db_session):
...
@pytest.mark.asyncio
async def test_login__send_req_without_required_data__return_error(async_client, db_session, req_filed):
...
@pytest.mark.asyncio
async def test_login__send_email_non_existent_user__return_401(async_client, db_session):
    resp = await async_client.post('/auth/token/', data=json.dumps(VALID_LOGIN_DATA))
    resp_json = resp.json()
